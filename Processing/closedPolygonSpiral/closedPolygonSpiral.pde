...
float x(float _a, float _t, int _n){
  return _a*cos(PI/_n)*cos((PI/_n)*(2*floor(_t)+1))-(2*_t-2*floor(_t)-1)*sin(PI/_n)*sin((PI/_n)*(2*floor(_t)+1));
}

float y(float _a, float _t, int _n){
  return _a*cos(PI/_n)*sin((PI/_n)*(2*floor(_t)+1))+(2*_t-2*floor(_t)-1)*sin(PI/_n)*cos((PI/_n)*(2*floor(_t)+1));
}

void keyPressed() {
  if (key == 'q') {
   saveFrame("closedPolygonSpiral-######.png");
   exit();
  }
}
